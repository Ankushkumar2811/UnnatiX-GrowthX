...
type AgentLive = { runs: AgentRun[]; leads: Lead[] };

export default function LeadsPage() {
  const router = useRouter();
  const [leads, setLeads] = useState<Lead[]>([]);
  const [agentLive, setAgentLive] = useState<AgentLive>({ runs: [], leads: [] });
  ...
  return <SafeAreaView style={s.safe} edges={['top']}>
    ...
    <ScrollView contentContainerStyle={s.scroll}>
      ...
      {latestRun ? <View style={s.runCard}>
      ...
      </View> : <View style={s.runCard}><Text style={s.runObjective}>No CEO-linked sales discovery run yet.</Text></View>}
